...
#if 0
void wait_child_process(int sig_no)
{

	printf("child process exit,signo=%d\n",sig_no);
	wait(NULL);
...
main(int argc, char *argv[])
{
    ...
    //if (clone(childFunc, stackTop, flags|SIGCHLD , &cp) == -1)
    //if (clone(childFunc, stackTop, flags | CHILD_SIG, &cp) == -1)
    //if (clone(childFunc, stackTop, flags |CLONE_NEWIPC|SIGCHLD, &cp) == -1)
    if (clone(childFunc, stackTop, flags |CLONE_NEWPID|CLONE_NEWIPC, &cp) == -1)
        errExit("clone");
